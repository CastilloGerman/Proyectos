...
class AppPresupuestos:
    ...
    def create_presupuestos_tab(self):
        ...
        self.plantilla_config_file = "plantilla_config.json"
        config = self.cargar_configuracion()
        self.carpeta_pdfs = config.get('carpeta_pdfs', os.getcwd())
        self.carpeta_facturas = config.get('carpeta_facturas', os.path.join(os.getcwd(), 'facturas'))
    ...
    def create_ver_presupuestos_tab(self):
        ...
        self.estado_combo = ttk.Combobox(filter_frame, textvariable=self.estado_filtro_var, 
                                       values=["Todos", "Pendiente", "Aprobado"], 
        ...
        ttk.Button(filter_frame, text="⏳ Marcar como Pendiente", command=self.marcar_pendiente, style='Warning.TButton').pack(side='left', padx=(5, 0))
        ...
        try:
            if os.path.exists(self.config_file):
                with open(self.config_file, 'r', encoding='utf-8') as f:
                    ...
                    carpeta_pdfs = config.get('carpeta_pdfs', os.getcwd())
                    if not os.path.exists(carpeta_pdfs):
                        carpeta_pdfs = os.getcwd()
                    
                    carpeta_facturas = config.get('carpeta_facturas', os.path.join(os.getcwd(), 'facturas'))
                    if not os.path.exists(carpeta_facturas):
                        carpeta_facturas = os.path.join(os.getcwd(), 'facturas')
    ...
    def cargar_configuracion_empresa(self):
        ...
        try:
            with open('plantilla_config.json', 'r', encoding='utf-8') as f:
                config = json.load(f)
            ...
            messagebox.showwarning("Advertencia", "No se encontró el archivo plantilla_config.json")
    ...
    def guardar_configuracion_empresa(self):
        ...
        try:
            ...
            # Guardar archivo
            with open('plantilla_config.json', 'w', encoding='utf-8') as f:
                json.dump(config, f, indent=2, ensure_ascii=False)
    ...
    def vista_previa_configuracion(self):
        ...
        try:
            ...
            archivo_preview = "vista_previa_configuracion_temp.pdf"

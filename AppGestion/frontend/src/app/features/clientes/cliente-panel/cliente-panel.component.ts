...
export class ClientePanelComponent implements OnInit {
  ...
  private ejecutarCambioEstadoPresupuesto(id: number, nuevo: string, estadoActual: string): void {
    ...
    this.presupuestoService.updateEstado(id, { estado: nuevo }).subscribe({
      next: (updated) => {
  ...
  private patchFacturaEstado(id: number, nuevo: string, montoParcial?: number): void {
    this.actualizandoFacturaId = id;
    this.facturaService.updateEstadoPago(id, { estadoPago: nuevo, montoCobrado: montoParcial }).subscribe({
      next: (updated) => {
        this.actualizandoFacturaId = null;
        this.refrescarPanel();
        this.snackBar.open(
